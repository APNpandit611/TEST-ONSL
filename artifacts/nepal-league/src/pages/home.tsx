...
function parseTournamentDate(
    ...
    if (!date) return new Date(2026, 5, 28, 10, 0, 0);
                    ...
                    <div className="px-4 py-5 md:py-6 grid grid-cols-[1fr_auto_1fr] items-center gap-4 md:gap-6">
                        ...
                        <div className="flex items-center justify-end gap-3 min-w-0">
                            <TeamLogo
                                ...
                                size="lg"
                            />
                            <h2 className="text-xl md:text-2xl font-black truncate min-w-0">
                                {match.homeTeamName}
                            </h2>
                        </div>
                        ...
                        <div className="flex items-center justify-start gap-3 min-w-0">
                            <h2 className="text-xl md:text-2xl font-black text-left truncate min-w-0">
                                {match.awayTeamName}
                            </h2>
                            <TeamLogo
...
export default function Home() {
    ...
    const allTeamsList = Array.isArray(allTeams) ? allTeams : [];

    const registeredIds = new Set(standingsList.map((s) => s.teamId));
    const registeredTeams = allTeamsList.filter((t) => registeredIds.has(t.id));

    // const registeredIds = new Set((standings ?? []).map((s) => s.teamId));
    // const registeredTeams = (allTeams ?? []).filter((t) =>
    //     registeredIds.has(t.id),
    // );

    // const registeredIds = useMemo(() => {
    //     if (!Array.isArray(standings)) return new Set<number>();
    //     return new Set(standings.map((s) => s.teamId));
    // }, [standings]);

    // const registeredTeams = useMemo(() => {
    //     if (!Array.isArray(allTeams)) return [];
    //     return allTeams.filter((t) => registeredIds.has(t.id));
    // }, [allTeams, registeredIds]);

    // Champion banner: final match finished → show winner
    // const finalMatch = (allMatches ?? []).find(
    //     (m) => m.matchType === "final" && m.status === "finished",
    // );

    const matchesList = Array.isArray(allMatches) ? allMatches : [];
    const liveMatchesList = Array.isArray(liveMatches) ? liveMatches : [];

    const finalMatch = matchesList.find(
        (m) => m.matchType === "final" && m.status === "finished",
    );
    ...
    const displayName =
        tournament?.name ?? "Ostrobothnia Nepal Super League 2026";
                    ...
                    <div className="bg-primary text-primary-foreground rounded-xl p-4 flex items-center justify-between cursor-pointer hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-bold uppercase tracking-wider">
                                ...
                                {liveMatches.length > 1 ? "es" : ""} Happening
                                Now
                            </span>
                        </div>
            ...
            <div className="relative overflow-hidden rounded-2xl border bg-card shadow-sm">
                ...
                <div className="relative p-6 md:p-12 flex flex-col items-center text-center space-y-6">
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                        <Badge
                            variant="outline"
                            className="border-primary/40 text-primary font-bold uppercase tracking-widest px-4 py-1.5 text-xs"
                        >
                            🏆 Hosting the Tournament
                        </Badge>
                        <Badge
                            variant="outline"
                            className="border-yellow-500/50 text-yellow-400 font-bold uppercase tracking-widest px-4 py-1.5 text-xs bg-yellow-500/10"
                        >
                            ⭐ Defending Champions
                        </Badge>
                    </div>
                    ...
                    <div className="flex flex-wrap justify-center gap-3 text-sm font-medium text-muted-foreground">
                        ...
                        <span className="flex items-center gap-2 bg-background border px-3 py-1.5 rounded-full">
                            ...
                            {registeredTeams.length}/{displayMaxTeams} Teams ·
                            Round-Robin
                        </span>
                    </div>
                    ...
                    {!tournamentStarted && (
                        <div className="w-full space-y-5">
                            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center justify-center gap-2">
                                <Clock className="h-4 w-4" /> Tournament starts
                                in
                            </p>
                ...
                <div className="relative overflow-hidden rounded-2xl border-2 border-amber-500/40 bg-amber-500/5 shadow-xl shadow-amber-500/10">
                    ...
                    <div className="relative p-6 md:p-12 flex flex-col items-center text-center space-y-5">
                        ...
                        <p className="text-muted-foreground max-w-md text-sm">
                            Congratulations to {winnerTeam.name} for winning the
                            championship final! A brilliant display of football
                ...
                <CardContent className="p-6 md:p-8 flex flex-col md:flex-row items-center gap-6">
                    <div className="flex-1 space-y-2">
                        ...
                        <h3 className="text-xl md:text-2xl font-black">
                            Is your club joining {displayShortName}?
                        </h3>
                        <p className="text-muted-foreground text-sm">
                            Register your team and squad before the tournament.
                            You'll need a team manager email for OTP
                            verification and at least 7 players.
                        </p>
                    </div>
                    ...
                    <CardContent className="p-6 space-y-3">
                        ...
                        <div>
                            <p className="font-bold text-lg">
                                {tournament?.venue ??
                                    "Santahaka Tekonurmikenttä"}
                            </p>
                            <p className="text-sm text-muted-foreground">
                                {tournament?.city ?? "Kokkola, Finland"}
                            </p>
                        </div>
                    ...
                    <Card className="border-dashed">
                        <CardContent className="p-8 text-center text-muted-foreground">
                            ...
                            <p className="text-sm mt-1">
                                Teams will appear here once they register their
                                squad.
                            </p>
                        </CardContent>
                    </Card>
                                    ...
                                    <Link key={match.id} href={linkHref}>
                                        ...
                                        >
                                            <CardContent className="p-0">
                                                {/* Header bar */}
                                                <div
                                                    className={`flex items-center justify-between px-4 py-2 text-xs font-bold uppercase tracking-wider text-white ${
                                                        isFinal
                                                            ? "bg-amber-500"
                                                            : isLive
                                                              ? "bg-primary"
                                                ...
                                                <div className="px-4 py-5 md:py-6 grid grid-cols-[1fr_auto_1fr] items-center gap-4 md:gap-8">
                                                    ...
                                                    <div className="flex items-center justify-end gap-3 min-w-0">
                                                        <div className="text-right min-w-0 space-y-0.5">
                                                            <p className="font-bold text-sm md:text-lg truncate leading-tight">
                                                                {
                                                                    match.homeTeamName
                                                                }
                                                            </p>
            ...
            <Card className="border-dashed">
                <CardContent className="p-6 flex flex-col md:flex-row items-center gap-6">
                    ...
                    <div className="text-center md:text-left space-y-2">
                        ...
                        <p className="text-muted-foreground text-sm">
                            Scan to follow live scores, fixtures and standings
                        ...
                        <div className="flex flex-wrap gap-3 justify-center md:justify-start pt-1">
                            <a
                                href="https://www.facebook.com/people/Kokkola-Soccer-Boys/61589834992626/"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors"
                            >
                                <Facebook className="h-3.5 w-3.5" /> Follow us
                                on Facebook
                            </a>
                            <a
                                href="mailto:info@kokkolasoccerboys.cc"
                                className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors"
                            >
                                <Mail className="h-3.5 w-3.5" />{" "}
                                info@kokkolasoccerboys.cc
                            </a>
                            <a
                                href="tel:+358413174494"
                                className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors"
                            >
                                <Phone className="h-3.5 w-3.5" /> +358 413 174
                                494
                            </a>
                        </div>
                    </div>
